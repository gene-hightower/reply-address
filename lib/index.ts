...
function decodeBlob(addr: string, secret: string): FromTo | undefined {
    const pkt = base32Decode(addr, base32Type);
...
function encodeBlob(replyInfo: FromTo, secret: string): string {
    ...
    console.log(`pkt === ${pkt}`);
    return `rep=${base32Encode(Buffer.fromString(pkt), base32Type)}`;
}

function isPureBase32(s: string): boolean {
    const is = /^[0-9A-Ha-hJ-Kj-kM-Nm-nP-Tp-tV-Zv-z]+$/.test(s);
    //console.log(`${s} is === ${is}`);
    return is;
}
...
export function decodeReply(addr: string, secret: string): FromTo | undefined {
    ...
    if (firstSep == lastSep || secondSep == lastSep) {
        console.log(`unrecognized reply format ${addr}`);
        return;
    }
    ...
    return { mailFrom: `${mailFromLoc}@${mailFromDom}`,
